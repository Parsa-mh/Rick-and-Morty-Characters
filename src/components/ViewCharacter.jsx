...
const ViewCharacter = ({findCharacter}) => {
    ...
    const {info,data,body} = styles
    return character ? (
        <div style={{height : "100vh", width : "100%"}} className={body}>
                ...
                <div className={`col-11 col-md-9 col-lg-6 mx-auto d-flex flex-row bg-light`} style={{borderRadius : "18px", boxShadow : "5px 5px 25px black"}}>
                    <div style={{ width: "55%" }} className="pt-3 d-flex flex-column align-items-center justify-content-evenly">
                        ...
                        <div className="d-flex flex-column align-items-center justify-content-around w-100" >
                            <p className={`text-center ${info} ${data}`}>species : {character.species}</p>
                            <p className={`text-center ${info} ${data}`}>origin : {character.origin.name.split(" ")[0]}</p>
                            <p className={`text-center ${info} ${data}`}>gender : {character.gender}</p>
                            <p className={`text-center ${info} ${data}`}>status : {character.status}</p>
                        </div>
                    </div>
